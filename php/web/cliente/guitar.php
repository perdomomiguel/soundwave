...
<head>
    ...
    <title>Soundwave | Guitarras</title>
...
<h1 class="popular-title1">Fender</h1>
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $sql2 = "SELECT * FROM compras WHERE id_producto=? AND id_cliente=?";
        $stmt2 = $conn->prepare($sql2);
        $stmt2->bind_param("ii", $id_producto, $id_cliente);
        $stmt2->execute();
        ...
        if (mysqli_num_rows($resultado) > 0) {
            ...
            if (isset($_SESSION["nombre"])):
                ...
                echo '<button class="cart" name="cart" onclick="disableButton(this, \'cart-' . $row['id_producto'] . '\')">';
            ...
            echo '<button class="fav" name="fav" onclick="disableButton(this, \'fav-' . $row['id_producto'] . '\')">';
            echo '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"';
...
<script>
function disableButton(button, id) {
    var cartButton = document.getElementById('cart-' + id);
    var favButton = document.getElementById('fav-' + id);
    
    if (button === cartButton) {
        
        cartButton.disabled = true;
        cartButton.classList.add('disabled');
    } else if (button === favButton) {
...
<h1 class="guitar-title2">Gibson</h1>
<div class="popular-section">

<?php
// Selecciona los datos de la tabla productos
$sql = "SELECT * FROM productos WHERE categoria = 'Guitarras Gibson'";
...
<h1 class="guitar-title2">Ibáñez</h1>
<div class="popular-section">

<?php
// Selecciona los datos de la tabla productos
$sql = "SELECT * FROM productos WHERE categoria = 'Guitarras Ibáñez'";
$result = mysqli_query($conn, $sql);
...
<style>
    button:disabled {
  cursor: default;
  /* Agrega cualquier otro estilo deseado para los botones deshabilitados */
}
